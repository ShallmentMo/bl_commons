...
begin
  Groupdate::RelationBuilder.class_eval do
    private

    # https://github.com/ankane/groupdate/blob/3c717b8df69a6153c583809811b9ca9d56124c2b/lib/groupdate/relation_builder.rb#L25
    # 由于 DRDS 不支持 CONVERT_TZ，这边修改为按对字段进行时间增减
    def group_clause
      time_zone = @time_zone.tzinfo.name
      offset = "#{@time_zone.utc_offset / 3600}:0:0"
      ...
      query =
        ...
        when "MySQL", "Mysql2", "Mysql2Spatial", 'Mysql2Rgeo'
          case period
          when :day_of_week
            ["DAYOFWEEK(ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?)) - 1", offset]
          when :day_of_year
            ["DAYOFYEAR(ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?))", offset]
          when :hour_of_day
            ["(EXTRACT(HOUR from ADDTIME(#{column}, ?)) + 24 - #{day_start / 3600}) % 24", offset]
          when :minute_of_hour
            ["(EXTRACT(MINUTE from ADDTIME(#{column}, ?)))", offset]
          when :day_of_month
            ["DAYOFMONTH(ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?))", offset]
          when :month_of_year
            ["MONTH(ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?))", offset]
          when :week
            ["SUBTIME(DATE_FORMAT(ADDTIME(DATE_SUB(#{column}, INTERVAL ((#{7 - week_start} + WEEKDAY(ADDTIME(#{column}, ?) - INTERVAL #{day_start} second)) % 7) DAY) - INTERVAL #{day_start} second, ?), '%Y-%m-%d 00:00:00') + INTERVAL #{day_start} second, ?)", offset, offset, offset]
          when :quarter
            ["DATE_ADD(SUBTIME(DATE_FORMAT(DATE(CONCAT(EXTRACT(YEAR FROM ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?)), '-', LPAD(1 + 3 * (QUARTER(ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?)) - 1), 2, '00'), '-01')), '%Y-%m-%d %H:%i:%S'), ?), INTERVAL #{day_start} second)", offset, offset, offset]
          else
            format =
              case period
              when :second
            ...
            ["DATE_ADD(SUBTIME(DATE_FORMAT(ADDTIME(DATE_SUB(#{column}, INTERVAL #{day_start} second), ?), '#{format}'), ?), INTERVAL #{day_start} second)", offset, offset]
          end
